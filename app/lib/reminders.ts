...
import { dateKeyInZone, isOccurrenceEligible, occurrencesOnOrAfter } from "./recurringProjection";
...
export function buildReminderGroups(captures: Capture[], todayKey: string): ReminderGroups {
  ...
  const occurrences = occurrencesOnOrAfter(eligible, todayKey, throughYear);
  ...
  for (const occurrence of occurrences) {
    ...
    if (dateKey > outerEndKey) continue;
